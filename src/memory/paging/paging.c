...
// Prototype so we can access this assembler function
...
struct paging_4gb_chunk* paging_new_4gb(uint8_t flags)
{
    ...
    for (int i = 0; i < PAGING_TOTAL_ENTRIES_PER_TABLE; i++)
    {
        ...
        {
            entry[b] = (offset + (b * PAGING_PAGE_SIZE)) | flags | PAGING_IS_WRITEABLE;
        }
...
// --------------------------------------------------------------
// Section from page 191 onwards: Building on our Paging Functionality
// --------------------------------------------------------------
...
int paging_map_to(uint32_t* directory, void* virt, void* phys, int flags)
{
    int res = 0;
    ...
    // We are mapping only one page at a time
    int total_pages = 1;
...
int paging_map_range(uint32_t* directory, void* virt, void* phys, int total_pages, int flags)
{
    ...
    {
        res = paging_map_to(directory, virt, phys, flags);
        if (res < 0)
            break;
